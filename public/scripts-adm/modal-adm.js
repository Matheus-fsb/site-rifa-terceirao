...
function showNewRegistrationModal(event, numberId, isAlteration = false) {
    ...
    form_completo.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch(`${process.env.BASE_URL}/buyers`, {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                const updateResponse = await fetch(`${process.env.BASE_URL}/numbers/${numberId}`, {
                    method: 'PATCH',
                    headers: createAuthHeaders(),
...
function showExistingRegistrationModal(event, numberId) {
    ...
    form_existente.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch(`${process.env.BASE_URL}/buyers/${formData.email}`, {
                method: 'GET',
                headers: createAuthHeaders(),
            });
...
deleteBuyerButton.addEventListener('click', async (event) => {
    ...
    if (confirmDeletion) {
        try {
            const response = await fetch(`${process.env.BASE_URL}/numbers-null/${numberId}`, {
                method: 'PATCH',
                headers: createAuthHeaders(),
            });
