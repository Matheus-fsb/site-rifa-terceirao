...
async function deleteNumbers(quantity) {
  try {
    ...
    const responseMax = await fetch(`${process.env.BASE_URL}/numbers'`);
    ...
    for (let i = maxNumber; i > maxNumber - numbersToDelete; i--) {
      const deleteResponse = await fetch(`${process.env.BASE_URL}/numbers/${i}`, {
        method: 'DELETE',
...
async function createNumbers(quantity) {
  try {
    ...
    for (let i = 0; i < numbers.length; i++) {
      const response = await fetch(`${process.env.BASE_URL}/numbers`, {
        method: 'POST',
        headers: createAuthHeaders(),
